...
struct my_triangle_3DModel
{
	...
	float reflection = 0;
	//以下用于计算直接光照能量
	float material_ambient_rgb_reflection[3] ; //环境光反射系数，初始不反射
	float material_diffuse_rgb_reflection[3] ;//漫反射系数，初始不反射
	float material_specular_rgb_reflection[3] ;//镜面光反射系数，初始不反射
		...
		while (!f.eof())
		{
			...
			if (parameters.size() == 4 || parameters.size() == 3)
			{
				...
				else if (parameters[0] == "f")
				{
					...
					int i = atoi(parameters[1].substr(0,
						parameters[1].find_first_of('/')).c_str());
					if (i == 0)i = 1;
					curTri.first_point_texture_index = i - 1;
					parameters[1] = parameters[1].substr(parameters[1].find_first_of('/') + 1);
					...
					int j = atoi(parameters[1].substr(0,
						parameters[1].find_first_of('/')).c_str());
					if (j == 0)j = 1;
					curTri.second_point_texture_index = j - 1;
					parameters[2] = parameters[2].substr(parameters[2].find_first_of('/') + 1);
					...
					int k = atoi(parameters[1].substr(0,
						parameters[1].find_first_of('/')).c_str());
					if (k == 0)k =1;
					curTri.third_point_texture_index = k - 1;
					parameters[3] = parameters[3].substr(parameters[3].find_first_of('/') + 1);
